...
public class ActivityMeetings extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        /*if (classGlobalApp.GetParam("loginNotFirstTime").equals("trueTrue")) {
            ...
            classGlobalApp.Log(getClass().getSimpleName(), "onCreate", "Ранее уже входили под настоящим логином", false);

            if (classGlobalApp.GetParam("requestIsActive").equals("trueTrue")) {// если заявка активна и заполнялась

                classGlobalApp.Log(getClass().getSimpleName(), "onCreate", "Заявка активна", false);
                ...
                classGlobalApp.Log(getClass().getSimpleName(), "onCreate", "Заявка не активна, загружаем заполнение заявки", false);
                ChangeFragment(fragmentRequestMeeting, false); // показываем заявку
            }
        } else { //если логинемся в первый раз, то нужно залезть на сервак и проверить ли активна ли заявка, если активна, то подгрузить ее на устройство

            classGlobalApp.Log(getClass().getSimpleName(), "onCreate", "Ранее не входили под настоящим логином", false);
                ...
                public void onComplete(@NonNull Task<DocumentSnapshot> task) { // как задача чтения выполнилась
                    if (task.isSuccessful()) { // если выполнилась успешно
                        ...
                        if (document.exists()) { // если документ такой есть, не null
                            ...
                            //ModelSingleMeeting modelSingleMeeting = (ModelSingleMeeting) document.getData();
                            //ModelSingleMeeting modelSingleMeeting = document.toObject(ModelSingleMeeting.class);

                            classGlobalApp.Log("FragmentDetailsMeeting", "onStart/onComplete", "Количество полей в документе = " + Integer.toString(mapDocument.size()), false);
